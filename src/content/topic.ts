...
export type TopicMeta = {
   ...
   * 내 경험. 있으면 글이 확 살아난다.
   *
   * **분위기가 아니라 사실의 목록이다.** 여기 적힌 것 밖의 이력은 모델이
   * 만들어 내면 안 된다 — 안 겪은 일을 1인칭으로 쓰면 그 글은 거짓말이 된다.
   * 피부 타입처럼 글마다 흔들리면 안 되는 값도 여기 적는다.
   */
  experience?: string;
  /** 링크 유도를 넣을지. 기본 false */
  withLink?: boolean;
...
export function buildTopicPrompt(
  ...
  const system = [
    ...
    /**
     * 경험을 한 줄에 끼워 넣지 않고 **따로 세운다.**
     *
     * `타겟 / 말투 / 경험` 을 한 줄로 붙여 두면 모델이 경험을 '분위기'로 읽고
     * 없는 이력을 채워 넣는다. 실제로 "2년 넘게", "고등학생 때부터" 가 나왔다.
     * 경험은 분위기가 아니라 **사실의 목록**이고, 목록에 없으면 없는 것이다.
     */
    ...(meta.experience
      ? [
          `[경험] ${meta.experience}`,
    ...
    "3개 글의 '나'는 **같은 사람이다.** 글마다 피부 타입·상황·이력이 달라지면 안 된다.",
    ...(meta.category
      ? [
